...
//interceptor
httpMovie.interceptors.request.use(
  (config) => {
    config = { ...config };
...
/* 
    ...
    200: Dữ liệu gởi đi và nhận về kết quả thành công (ok)
    201: Dữ lệu khởi tạo thành công (Created)
    400: Bad request (Lỗi không tìm tháy item trên backend)
    404: Not found (không tìm thấy link backend)
    500: Error in server (Lỗi xảy ra tại server - có thể do dữ liệu frontend gửi lên xử lý bị lỗi backend k catch trường hợp này thì ra 500 hoặc là backend code bị lỗi)=> xác định lỗi => mở postman request thử với data dùng thì có được hay không nếu vẫn lỗi thì bảo backend fix
